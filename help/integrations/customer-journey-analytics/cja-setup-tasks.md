---
title: Customer Journey AnalyticsとAdobe Advertising間でデータを交換するためのタスクを設定する
description: Adobe Customer Journey AnalyticsとAdobe Advertising間でデータを交換するために必要な重要なステップをご確認ください
level: Beginner, Intermediate, Experienced
topic: Integrations
feature: Integration with Adobe Customer Journey Analytics
hide: true
exl-id: 1d9ef24f-02bf-4260-9d16-24580cd1c7a3
TQID: https://experienceleague.adobe.com/EJ1EfQzqh5wKH1NzmHaOmC2BnFO4axdDVC6zKDGK1n0
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: aa2f3246-cb95-4b30-8899-fdf7d73550cc
  - id: cdd65e7e-8839-44a2-bc21-0e03623b5dd1
source-git-commit: 47f5a750dcff110344414bcae6c914d02ce71730
workflow-type: tm+mt
source-wordcount: 208
ht-degree: 0%

---

# Customer Journey AnalyticsとAdobe Advertising間でデータを交換するためのタスクを設定する

## Customer Journey Analytics コンバージョンイベントをAdobe Advertisingに送信するための初期設定タスク

Customer Journey AnalyticsのコンバージョンイベントをAdobe Advertisingに送信するために必要な最初の手順について説明します。後でコンバージョンイベントを最適化目標/目標に含めることができます。

これらの手順を完了し、Adobe アカウントチームにデータビューを提供したら、追加の手順を実行して統合を設定できます。

### 概要

>[!VIDEO](https://video.tv.adobe.com/v/3476810)

### 手順1:Customer Journey Analyticsで位置情報アカウントを設定する

>[!VIDEO](https://video.tv.adobe.com/v/3476811)

### 手順2：書き出す指標を使用してAnalysis Workspaceでデータビューを作成する

>[!VIDEO](https://video.tv.adobe.com/v/3476823)

>[!MORELIKETHIS]
>
>* [Adobe AdvertisingとCustomer Journey Analyticsの連携の概要](https://experienceleague.adobe.com/en/docs/advertising/integrations/customer-journey-analytics/overview)

## Customer Journey AnalyticsでAdobe Advertising キャンペーンデータを表示する手順

### 概要

>[!VIDEO](https://video.tv.adobe.com/v/3476847)

### 手順1:Customer Journey Analyticsでの接続の設定

>[!VIDEO](https://video.tv.adobe.com/v/3476860)

### 手順2:Customer Journey Analyticsでのデータビューの設定

>[!VIDEO](https://video.tv.adobe.com/v/3476872?learn=on)

### 手順3:Adobe Advertising ディメンションの永続性設定と派生フィールドの設定

>[!VIDEO](https://video.tv.adobe.com/v/3476897?learn=on)

>[!MORELIKETHIS]
>
>* [書面の文書](https://experienceleague.adobe.com/en/docs/advertising/integrations/customer-journey-analytics/overview)。
